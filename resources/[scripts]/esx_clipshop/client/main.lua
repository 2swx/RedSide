...
function OpenShopMenu(zone)
  ...
  table.insert(elements, {
      label = 'Chargeur' .. '<span style="color:green;">$' .. price .. ' </span>',
      value = 'clip'
  })  
  ...
  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop', {
    ...
    elements = elements
	  ...
	  if data.current.value == 'clip' then
	    TriggerServerEvent('esx_clipshop:buyChargeur', zone)
	  end
...
AddEventHandler('esx_clipshop:hasEnteredMarker', function(zone)
  CurrentAction     = 'shop_menu'
  CurrentActionMsg  = _U('shop_menu')
  CurrentActionData = {zone = zone}
end)

AddEventHandler('esx_clipshop:hasExitedMarker', function(zone)
  CurrentAction = nil
  ESX.UI.Menu.CloseAll()
end)
...
Citizen.CreateThread(function()
  while true do
    ...
    if isInMarker and not HasAlreadyEnteredMarker then
      ...
      TriggerEvent('esx_clipshop:hasEnteredMarker', currentZone)
    end
    if not isInMarker and HasAlreadyEnteredMarker then
      HasAlreadyEnteredMarker = false
      TriggerEvent('esx_clipshop:hasExitedMarker', LastZone)
    end
  end
end)
