...
local blips = {
 ...
 -- {title="~y~Vagos", colour=5, id=84, x = 340.93, y = -2045.12, z = 21.23}
  }
  Citizen.CreateThread(function()
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      if data.current.value == 'item' then
        TriggerServerEvent('esx_vagos:buyItem')
      end

      if data.current.value == 'item1' then
        TriggerServerEvent('esx_vagos:buyItem1')
      end

      if data.current.value == 'item2' then
        TriggerServerEvent('esx_vagos:buyItem2')
      end
...
function OpenGetStocksBanditsMenu()
  ...
  ESX.TriggerServerCallback('esx_vagos:getStockItemsBandits', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_vagos:getStockItemsBandits', itemName, count)
            end
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_vagos:putStockItemsBandits', itemName, count)
            end
  ...
  if Config.EnableArmoryManagement then
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        TriggerServerEvent('esx_vagos:giveWeaponBandits', weapon,  1000)
      end,
      function(data, menu)
...
function OpenGetWeaponBanditsMenu(station)
  ...
  ESX.TriggerServerCallback('esx_vagos:getArmoryWeaponsBandits', function(weapons)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        ESX.TriggerServerCallback('esx_vagos:removeArmoryWeaponBandits', function()
...
function OpenPutWeaponBanditsMenu(station)
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      ESX.TriggerServerCallback('esx_vagos:addArmoryWeaponBandits', function()
  ...
  ESX.TriggerServerCallback('esx_vagos:getArmoryWeaponsBandits', function(weapons)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        ESX.TriggerServerCallback('esx_vagos:buyBandits', function(hasEnoughMoney)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        if data.current.value == 'formol' then
          menu.close()
          TriggerServerEvent('esx_vagos:startHarvestBandits')
        end
      end,
      function(data, menu)
        menu.close()
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        if data.current.value == 'puce' then
          menu.close()
          TriggerServerEvent('esx_vagos:startHarvest2Bandits')
        end
      end,
      function(data, menu)
        menu.close()
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if data.current.value == 'spark' then
          menu.close()
          TriggerServerEvent('esx_vagos:startHarvest3Bandits')
        end

        if data.current.value == 'tesla' then
          menu.close()
          TriggerServerEvent('esx_vagos:startHarvest4Bandits')
        end
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        if data.current.value == 'sedatif' then
          menu.close()
          TriggerServerEvent('esx_vagos:startCraftBandits')
        end

        if data.current.value == 'cryptedphone' then
          menu.close()
          TriggerServerEvent('esx_vagos:startCraft2Bandits')
        end

        if data.current.value == 'lightbreak' then
          menu.close()
          TriggerServerEvent('esx_vagos:startCraft3Bandits')
        end
...
AddEventHandler('esx_vagos:hasEnteredMarkerBandits', function(zone, station, part, partNum)
...
AddEventHandler('esx_vagos:hasExitedMarkerBandits', function(zone, station, part, partNum)

  if zone == 'HarvestBandits' then
    TriggerServerEvent('esx_vagos:stopHarvestBandits')
  end

  if zone == 'HarvestBandits2' then
    TriggerServerEvent('esx_vagos:stopHarvest2Bandits')
  end  

  if zone == 'HarvestBandits3' then
    TriggerServerEvent('esx_vagos:stopHarvest3Bandits')
    TriggerServerEvent('esx_vagos:stopHarvest4Bandits')
  end
  
  if zone == 'CraftBandits' then
    TriggerServerEvent('esx_vagos:stopCraftBandits')
    TriggerServerEvent('esx_vagos:stopCraft2Bandits')
    TriggerServerEvent('esx_vagos:stopCraft3Bandits')
  end
  ...
  while true do
    ...
    if PlayerData.job2 ~= nil and PlayerData.job2.name == 'vagos' then
      ...
      if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastZone ~= currentZone or LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum) ) then
        ...
        then
          TriggerEvent('esx_vagos:hasExitedMarkerBandits', LastZone, LastStation, LastPart, LastPartNum)
          hasExited = true
        end
        ...
        TriggerEvent('esx_vagos:hasEnteredMarkerBandits', currentZone, currentStation, currentPart, currentPartNum)
      end
...
Citizen.CreateThread(function()
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlPressed(0,  Keys['E']) and PlayerData.job2 ~= nil and PlayerData.job2.name == 'vagos' and (GetGameTimer() - GUI.Time) > 300 then
...
function OpenGetBlackMoney()
  ...
  ESX.TriggerServerCallback('esx_vagos:getBlackMoneySociety', function(inventory)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        if data.current.type == 'item_weapon' then
          menu.close()
          TriggerServerEvent('esx_vagos:getItem', data.current.type, data.current.value, data.current.ammo)
          ...
          ESX.UI.Menu.Open(
            ...
            function(data2, menu)
              ...
              if quantity == nil then
                ...
                menu.close()
                TriggerServerEvent('esx_vagos:getItem', data.current.type, data.current.value, quantity)
                ESX.SetTimeout(300, function()
                  OpenGetBlackMoney()
                end)
              end
...
function OpenPutBlackMoney()
  ...
  ESX.TriggerServerCallback('esx_vagos:getPlayerInventory2', function(inventory)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if data.current.type == 'item_weapon' then
          ...
          TriggerServerEvent('esx_vagos:putItem', data.current.type, data.current.value, data.current.ammo)
          ...
          ESX.UI.Menu.Open(
            ...
            function(data2, menu)
              ...
              TriggerServerEvent('esx_vagos:putItem', data.current.type, data.current.value, tonumber(data2.value))
