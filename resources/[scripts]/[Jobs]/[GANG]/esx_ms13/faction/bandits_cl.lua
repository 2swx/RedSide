...
--  {title="~y~QG MS13", colour=5, id=84, x = -64.88, y = 821.65, z = 231.65}
  }
  Citizen.CreateThread(function()
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
            ...
            ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
              ...
              ESX.UI.Menu.Open(
                ...
                function(data, menu)
                  ...
                  TriggerServerEvent('esx_society:removeVehicleFromGarage', 'ms13', vehicleProps)
      ...
      if data.current.value == 'item' then
        TriggerServerEvent('esx_ms13:buyItem')
      end

      if data.current.value == 'item1' then
        TriggerServerEvent('esx_ms13:buyItem1')
      end

      if data.current.value == 'item2' then
        TriggerServerEvent('esx_ms13:buyItem2')
      end
      ...
      if data.current.value == 'boss_actions' then
        TriggerEvent('esx_society:openBossMenu', 'ms13', function(data, menu)
          menu.close()
        end, {wash = false})
      end
...
function OpenGetStocksBanditsMenu()
  ...
  ESX.TriggerServerCallback('esx_ms13:getStockItemsBandits', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_ms13:getStockItemsBandits', itemName, count)
            end
...
ESX.TriggerServerCallback('esx_ms13:getPlayerInventory', function(inventory)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_ms13:putStockItemsBandits', itemName, count)
            end
  ...
  if Config.EnableArmoryManagement then
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        TriggerServerEvent('esx_ms13:giveWeaponBandits', weapon,  1000)
      end,
      function(data, menu)
...
function OpenGetWeaponBanditsMenu(station)
  ...
  ESX.TriggerServerCallback('esx_ms13:getArmoryWeaponsBandits', function(weapons)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        ESX.TriggerServerCallback('esx_ms13:removeArmoryWeaponBandits', function()
...
function OpenPutWeaponBanditsMenu(station)
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      ESX.TriggerServerCallback('esx_ms13:addArmoryWeaponBandits', function()
  ...
  ESX.TriggerServerCallback('esx_ms13:getArmoryWeaponsBandits', function(weapons)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        ESX.TriggerServerCallback('esx_ms13:buyBandits', function(hasEnoughMoney)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        if data.current.value == 'formol' then
          menu.close()
          TriggerServerEvent('esx_ms13:startHarvestBandits')
        end
      end,
      function(data, menu)
        menu.close()
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        if data.current.value == 'puce' then
          menu.close()
          TriggerServerEvent('esx_ms13:startHarvest2Bandits')
        end
      end,
      function(data, menu)
        menu.close()
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if data.current.value == 'spark' then
          menu.close()
          TriggerServerEvent('esx_ms13:startHarvest3Bandits')
        end

        if data.current.value == 'tesla' then
          menu.close()
          TriggerServerEvent('esx_ms13:startHarvest4Bandits')
        end
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        if data.current.value == 'sedatif' then
          menu.close()
          TriggerServerEvent('esx_ms13:startCraftBandits')
        end

        if data.current.value == 'cryptedphone' then
          menu.close()
          TriggerServerEvent('esx_ms13:startCraft2Bandits')
        end

        if data.current.value == 'lightbreak' then
          menu.close()
          TriggerServerEvent('esx_ms13:startCraft3Bandits')
        end
...
AddEventHandler('esx_ms13:hasEnteredMarkerBandits', function(zone, station, part, partNum)
...
AddEventHandler('esx_ms13:hasExitedMarkerBandits', function(zone, station, part, partNum)

  if zone == 'HarvestBandits' then
    TriggerServerEvent('esx_ms13:stopHarvestBandits')
  end

  if zone == 'HarvestBandits2' then
    TriggerServerEvent('esx_ms13:stopHarvest2Bandits')
  end  

  if zone == 'HarvestBandits3' then
    TriggerServerEvent('esx_ms13:stopHarvest3Bandits')
    TriggerServerEvent('esx_ms13:stopHarvest4Bandits')
  end
  
  if zone == 'CraftBandits' then
    TriggerServerEvent('esx_ms13:stopCraftBandits')
    TriggerServerEvent('esx_ms13:stopCraft2Bandits')
    TriggerServerEvent('esx_ms13:stopCraft3Bandits')
  end
  ...
  while true do
    ...
    if PlayerData.job2 ~= nil and PlayerData.job2.name == 'ms13' then
      ...
      if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastZone ~= currentZone or LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum) ) then
        ...
        then
          TriggerEvent('esx_ms13:hasExitedMarkerBandits', LastZone, LastStation, LastPart, LastPartNum)
          hasExited = true
        end
        ...
        TriggerEvent('esx_ms13:hasEnteredMarkerBandits', currentZone, currentStation, currentPart, currentPartNum)
      end
...
Citizen.CreateThread(function()
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlPressed(0,  Keys['E']) and PlayerData.job2 ~= nil and PlayerData.job2.name == 'ms13' and (GetGameTimer() - GUI.Time) > 300 then
        ...
        -- if CurrentAction == 'delete_vehicle' then
          ...
          -- if hash == GetHashKey('oracle') or hash == GetHashKey('r8prior') or hash == GetHashKey('italia458') or hash == GetHashKey('w124') or hash == GetHashKey('baller6')then
           -- if Config.MaxInService ~= -1 then
              -- TriggerServerEvent('esx_service:disableService', 'ms13')
           -- end
...
AddEventHandler('esx_ms13:teleportMarkers', function(position)
  ...
  -- while true do
    ...
    -- if PlayerData.job2 ~= nil and PlayerData.job2.name == 'ms13' then
        ...
        -- if IsControlJustReleased(0, Keys["E"]) and isInPublicMarker then
          -- TriggerEvent('esx_ms13:teleportMarkers', position)
        -- end
...
function OpenGetBlackMoney()
  ...
  ESX.TriggerServerCallback('esx_ms13:getBlackMoneySociety', function(inventory)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        if data.current.type == 'item_weapon' then
          menu.close()
          TriggerServerEvent('esx_ms13:getItem', data.current.type, data.current.value, data.current.ammo)
          ...
          ESX.UI.Menu.Open(
            ...
            function(data2, menu)
              ...
              if quantity == nil then
                ...
                menu.close()
                TriggerServerEvent('esx_ms13:getItem', data.current.type, data.current.value, quantity)
                ESX.SetTimeout(300, function()
                  OpenGetBlackMoney()
                end)
              end
...
function OpenPutBlackMoney()
  ...
  ESX.TriggerServerCallback('esx_ms13:getPlayerInventory2', function(inventory)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if data.current.type == 'item_weapon' then
          ...
          TriggerServerEvent('esx_ms13:putItem', data.current.type, data.current.value, data.current.ammo)
          ...
          ESX.UI.Menu.Open(
            ...
            function(data2, menu)
              ...
              TriggerServerEvent('esx_ms13:putItem', data.current.type, data.current.value, tonumber(data2.value))
