...
ESX                           = nil
animBind = NativeUI.CreateMenu("RedSide", "", 0, 0)
    ...
    pref.OnListSelect = function(_, item, index)
        ...
        if item == bind then
            if selectedBindAnim ~= nil then
                TriggerEvent("dqp:shownotif", "Touches d\'animations : " .. selectedBindAnim .. "~n~sur la touche " .. pavInd[index], 26)
                TriggerServerEvent("dqp:saveBind", selectedAnim, pavName[index])
            else
                TriggerEvent("dqp:shownotif", "Aucune animation selectionne !", 6)
            end
        end
        if item == animq then
            TriggerEvent("dqp:shownotif", "Animation Choisi : " .. pavAnim[index], 26)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, Keys['W']) and IsInputDisabled(0) and not isDead then
...
RegisterNetEvent('dqp:ready')
AddEventHandler('dqp:ready', function()
    ESX.TriggerServerCallback("dqp:getsettings", function(result)
        ...
        if result[1] ~= nil then
            ...
            TriggerEvent("dqp:shownotif", "Touche enregistrée", 26)
        end
    end)
end)
