...
function AddShopsMenu(menu)
    ...
    if item == eau then
        ...
        Citizen.Wait(1)
        ESX.ShowAdvancedNotification("Superette", "~b~Ticket Client", "Vous avez acheté de l' ~b~Eau", "CHAR_MP_ROBERTO", 1)
    elseif item == cocacola then
        TriggerServerEvent('buyCocaCola')
        ESX.ShowNotification('Vous avez payez ~r~3$')
        Citizen.Wait(1)
        ESX.ShowAdvancedNotification("Superette", "~b~Ticket Client", "Vous avez acheté du ~b~CocaCola", "CHAR_MP_ROBERTO", 1)
    elseif item == icetea then
        TriggerServerEvent('buyIceTea')
        ESX.ShowNotification('Vous avez payez ~r~5$')
        Citizen.Wait(1)
        ESX.ShowAdvancedNotification("Superette", "~b~Ticket Client", "Vous avez acheté du ~b~IceTea", "CHAR_MP_ROBERTO", 1)
    elseif item == fanta then
        TriggerServerEvent('buyfanta')
        ESX.ShowNotification('Vous avez payez ~r~6$')
        Citizen.Wait(1)
        ESX.ShowAdvancedNotification("Superette", "~b~Ticket Client", "Vous avez acheté du ~b~Fanta", "CHAR_MP_ROBERTO", 1)
    elseif item == jusfruit then
        TriggerServerEvent('buyjusfruit')
        ESX.ShowNotification('Vous avez payez ~r~4$')
        Citizen.Wait(1)
        ESX.ShowAdvancedNotification("Superette", "~b~Ticket Client", "Vous avez acheté du ~b~Jus de Fruit", "CHAR_MP_ROBERTO", 1)
        end
    end
    ...
    if item == pain then
        ...
        ESX.ShowNotification('Vous avez payez ~r~3$')
        Citizen.Wait(1)
        ESX.ShowAdvancedNotification("Superette", "~b~Ticket Client", "Vous avez acheté du ~b~Pain", "CHAR_MP_ROBERTO", 1)
    elseif item == chips then
        TriggerServerEvent('buyChips')
        ESX.ShowNotification('Vous avez payez ~r~4$')
        Citizen.Wait(1)
        ESX.ShowAdvancedNotification("Superette", "~b~Ticket Client", "Vous avez acheté des ~b~Chips BBQ", "CHAR_MP_ROBERTO", 1)
    elseif item == cheesebows then
        TriggerServerEvent('buycheesebows')
        ESX.ShowNotification('Vous avez payez ~r~5$')
        Citizen.Wait(1)
        ESX.ShowAdvancedNotification("Superette", "~b~Ticket Client", "Vous avez acheté des ~b~Chips Fromage", "CHAR_MP_ROBERTO", 1)    
    end
end    
    ...
    if item == gym_membership then
        ...
        Citizen.Wait(1)
        ESX.ShowAdvancedNotification("Superette", "~b~Ticket Client", "Vous avez acheté un abonnement ~b~Musculation", "CHAR_MP_ROBERTO", 1)
        end
    end
end
