...
function SendWebhookMessagePetrolDagan(webhook,message)
	webhook = "https://discordapp.com/api/webhooks/627203187367215106/Ek-fNkbywAVNeDGDeQpxDzmfcERCDjLGQhw2OTdAL0ZdlCu4hLygc-QvOKM8s_dl_tt3"
	if webhook ~= "none" then
		PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({content = message}), { ['Content-Type'] = 'application/json' })
	end
...
function SendWebhookScreenMessagePetrolDagan(webhook,message)
	webhook = "https://discordapp.com/api/webhooks/627203187367215106/Ek-fNkbywAVNeDGDeQpxDzmfcERCDjLGQhw2OTdAL0ZdlCu4hLygc-QvOKM8s_dl_tt3"
	if webhook ~= "none" then
		PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({content = message}), { ['Content-Type'] = 'application/json' })
	end
...
RegisterServerEvent('log:runSociete')
AddEventHandler('log:runSociete', function(societe, montantEntreprise, montantEmploye)
	name = GetPlayerName(source)
	if montantEntreprise == nil then
		montantEntreprise = 'X'
	end
	if montantEmploye == nil then
		montantEmploye = 'X'
	end
	SendWebhookScreenMessagePetrolDagan(webhook,"**Run**:\nEmployé: **"..name.."**\nMontant entreprise: **"..montantEntreprise.."**$\nMontant employé: **"..montantEmploye.."**$")
end)
...
AddEventHandler('log:ArgentSociete', function(depotsOuRetire, societe, motant)
	...
	if societe == 'fueler' then
		SendWebhookMessagePetrolDagan(webhook,"**Comptable:**\nEmployé: **"..name.."**\n"..depotsOuRetire.."\nMontant: **"..motant.."**$")
	end
end)
